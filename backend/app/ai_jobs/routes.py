...
async def create_job(
    ...
    # generate/explain/repair/suggest only produce suggestions/text -- even
    # when scoped to a real connection via connection_id, they only read its
    # schema (viewer already has read access to run queries against it), so
    # viewer is enough there (mirrors "viewer can read" from files/
    # connections). investigate is different: it writes a new file into the
    # workspace's tree, so it needs the same editor+ bar as
    # files.routes.create_file, or a viewer could use it to write files
    # despite having no write access anywhere else.
    ...
    except service.InvalidConnectionError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
