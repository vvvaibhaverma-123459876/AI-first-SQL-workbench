...
/**
 * The pre-v2 single-user demo workbench, unchanged. Runs against the
 * bundled global demo database — not workspace- or connection-scoped, so
 * it's kept separate from (not embedded inside) a v2 workspace shell until
 * Phase 2 makes every AI feature connection-scoped for real. Reachable from
 * WorkspaceShell for anyone who wants the original demo experience.
 */
export function LegacyDemoWorkbench({ onExit }: { onExit: () => void }) {
    ...
    <div className="h-screen overflow-hidden bg-slate-950 p-3 text-slate-100">
      ...
      <div className="mb-3 flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/80 px-5 py-3 backdrop-blur">
        <div className="flex items-center gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border border-slate-700 bg-slate-800 px-2 py-0.5 text-[10px] text-slate-400">local-first · legacy demo</span>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button className="!border-0 !bg-transparent text-slate-400 hover:text-slate-200" onClick={onExit}>
            ← Back to workspace
          </button>
          {!backendConnected && (
