...
import { Bot, Copy, Play, Save, Sparkles, Wand2, Wrench } from 'lucide-react'
...
export function EditorPanel() {
  ...
  return (
    <div className="panel flex h-full flex-col p-4">
      ...
      <textarea value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} className="mb-3 w-full" rows={2} placeholder="Ask the local assistant: e.g. Which referral channel has the best card activation conversion?" />

      <div className="mb-3 flex flex-wrap items-center gap-2">
        <button disabled={loading} onClick={runSQL}><Play size={14} className="mr-1 inline" />Run SQL</button>
        <button disabled={assistantLoading} onClick={runAssistant}><Bot size={14} className="mr-1 inline" />Ask + Run</button>
        <button disabled={assistantLoading} onClick={generateSQL}><Sparkles size={14} className="mr-1 inline" />Generate Only</button>
